...
def adjust_targets(y):
    adjusted_y = []
    for i in range(len(y)):
        if i > 0 and y[i - 1] >= 100:
            adjusted_y.append(0)
        else:
            adjusted_y.append(y[i])
    return np.array(adjusted_y)

# Ajustement des cibles
y_train_adjusted = adjust_targets(y_train)
y_test_adjusted = adjust_targets(y_test)

# Utilisez les cibles ajustées lors de la création des séquences
seq_length = 15
X_seq_train, y_seq_train = create_sequences(X_combined_scaled_train, y_train_adjusted, seq_length)
X_seq_test, y_seq_test = create_sequences(X_combined_scaled_test, y_test_adjusted, seq_length)
