...
class GaitPhaseEstimator:
    def __init__(self):
        ...
        self.phase_pub = rospy.Publisher('/stance_swing_phase_LSTM', Int16, queue_size=2)  # New publisher for stance/swing phase
    ...
    def mean_filter(self, predictions, window_size, anomaly_threshold=20):
        """
        ...
        """
        # Vérifier si predictions est une séquence iterable, sinon l'encapsuler dans une liste
    ...
    def estimate_phase(self):
        if (self.modelLoaded is not None) and (self.angleUpdated == True) and (self.forceUpdated == True):
            ...
            if len(self.data_sequence) == self.samples_size:
                ...
                # Perform prediction
                estimated_phase = self.model.predict(X, verbose=0)[0][0]
