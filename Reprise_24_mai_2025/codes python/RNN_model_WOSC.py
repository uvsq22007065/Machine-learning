...
class GaitPhaseEstimator:
    def __init__(self, data_folder, patient_id="subject4", samples_size=10):
        # Setup paths
        ...
        self.log_file_path = os.path.join(self.base_path, f"{self.patient}_modelRNN_log.txt")
        ...
        self.current_results_folder = os.path.join(self.results_folder, f"RNN_{patient_id}_final_training_{now}_with_prediction")
        if not os.path.exists(self.current_results_folder):
            os.makedirs(self.current_results_folder)

        self.labels_path = os.path.join(self.current_results_folder, f"{self.patient}_labelsRNN.csv")
        self.model_path = os.path.join(self.current_results_folder, f"{self.patient}_modelRNN.keras")
        self.scaler_path = os.path.join(self.current_results_folder, f"{self.patient}_modelRNN_scaler.pkl")
    ...
    def train_model(self, data_file, data_percentage):
        ...
        print("Last Train and Validation Loss for RNN+LSTM Model:")
        ...
        model_path_with_pct = os.path.join(self.current_results_folder, 
                                          f"{self.patient}_modelRNN_{data_percentage}pct")
        try:
        ...
        scaler_path_with_pct = os.path.join(self.current_results_folder, 
                                           f"{self.patient}_scalerRNN_{data_percentage}pct.pkl")
...
def main():
    ...
    # Fichier de données
    data_file = os.path.join(data_folder, "subject4_labelsRNN.csv")
    
    # Initialiser et entraîner le modèle
    estimator = GaitPhaseEstimator(data_folder, patient_id="subject4")
