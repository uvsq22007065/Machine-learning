...
class GaitPhaseEstimator:
    def __init__(self, samples_size=10):
        # ROS setup
        rospy.init_node('gait_phase_estimator_RNN', anonymous=True)
        ...
        self.labels_path = os.path.join(package_path, "log", "learning_models", f"{self.patient}_labelsRNN.xlsx")
        self.model_path = os.path.join(package_path, "log", "learning_models", f"{self.patient}_modelRNN.pkl")
        self.scaler_path = os.path.join(package_path, "log", "learning_models", f"{self.patient}_modelRNN_scaler.pkl")
        self.bag_path = os.path.join(package_path, "log", "training_bags", f"{self.patient}/{self.patient}_train_merged.bag")
        self.log_file_path = os.path.join(package_path, "log", "learning_models", f"{self.patient}_modelRNN_log.txt")
    ...
    def sub_pub_initialization(self):
        ...
        self.force_dt_pub = rospy.Publisher('/force_derivative_RNNs', Float64, queue_size=2)
        self.angle_dt_pub = rospy.Publisher('/angle_derivative_RNNs', Float64, queue_size=2)
        self.phase_pub = rospy.Publisher('/stance_swing_phase_RNNs', Int16, queue_size=2)  
    ...
    def train_model(self):
        ...
        rospy.loginfo("Training the RNN model...")
        ...
        plt.legend()
        loss_plot_path = os.path.join(os.path.dirname(self.model_path), f"{self.patient}_rnn_loss_plot.png")
    ...
    def estimate_phase(self):
        if self.modelLoaded and self.angleUpdated and self.forceUpdated:
            ...
            current_input = np.array(current_input_scaled).reshape(1, 1, 5)
        
            # Prediction
            new_phase = float(self.model.predict(current_input, verbose=0)[-1])  # Check for the last element
