...
class GaitPhaseEstimator:
    def __init__(self, samples_size=10):
        # ROS setup
        rospy.init_node('gait_phase_estimator_CNN', anonymous=True, log_level=rospy.INFO)
        ...
        self.labels_path = os.path.join(package_path, "log", "learning_models", f"{self.patient}_labelsCNN.xlsx")
        self.model_path = os.path.join(package_path, "log", "learning_models", f"{self.patient}_modelCNN.keras")
        self.scaler_path = os.path.join(package_path, "log", "learning_models", f"{self.patient}_modelCNN_scaler.pkl")
        self.bag_path = os.path.join(package_path, "log", "training_bags", f"{self.patient}/{self.patient}_train_merged.bag")
        self.log_file_path = os.path.join(package_path, "log", "learning_models", f"{self.patient}_modelCNN_log.txt")
        ...
        self.input_sequence = deque([], maxlen=self.sequence_length)
    ...
    def sub_pub_initialization(self):
        ...
        self.gait_ptg_pub = rospy.Publisher('/gait_percentage_CNNs', Int16, queue_size=2)
        self.force_dt_pub = rospy.Publisher('/force_derivative_CNNs', Float64, queue_size=2)
        self.angle_dt_pub = rospy.Publisher('/angle_derivative_CNNs', Float64, queue_size=2)
        self.phase_pub = rospy.Publisher('/stance_swing_phase_CNNs', Int16, queue_size=2)  
    ...
    def update_input_vector(self):
        """Builds and stores the input vector for prediction"""
        if self.angleUpdated and self.forceUpdated:
            ...
            self.input_sequence.append(input_vector)
    ...
    def train_model(self):
        ...
        # Training
        rospy.loginfo("Training the CNN+LSTM model...")
        ...
        plt.legend()
        loss_plot_path = os.path.join(os.path.dirname(self.model_path), f"{self.patient}_cnn_lstm_loss_plot.png")
        plt.savefig(loss_plot_path)
        plt.close()

        # Save Model
        rospy.loginfo(f"Saving CNN+LSTM model to {self.model_path}...")
    ...
    def estimate_phase(self):
        if self.modelLoaded and len(self.input_sequence) == self.sequence_length:
            current_sequence = np.array(self.input_sequence)
            current_sequence_scaled = self.scaler.transform(current_sequence)
            current_sequence_scaled = current_sequence_scaled.reshape(1, self.sequence_length, 5)

            # Prediction
            new_phase = float(self.model.predict(current_sequence_scaled, verbose=0)[-1])
    ...
    def run(self):
        ...
        if os.path.exists(self.model_path) and os.path.exists(self.scaler_path):
            rospy.loginfo(f"Model and scaler found for patient {self.patient}. Proceeding with phase estimation.")
            self.model = load_model(self.model_path)
            self.scaler = load(self.scaler_path)
            self.modelLoaded = True
        else:
            rospy.logwarn("Model or scaler not found. Training required.")
            res = self.train_model()
            if res == 0:
                rospy.signal_shutdown("Model was not found or scaler is missing.")
